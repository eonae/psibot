...
class TelegramMessageHandler:
        ...
        **kwargs
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(
                    "Rate limit hit, waiting %d seconds before retry. "
                    "Attempt %d/%d",
                    wait_time,
                    attempt + 1,
                    max_retries
    ...
    async def send_message(self, user_id: int, message: Message) -> None:
        try:
            ...
            logger.error("Unexpected error while sending message to user %d: %s", user_id, e)
            raise
    ...
    async def send_document(
        ...
        try:
            ...
            logger.error("Unexpected error while sending document to user %d: %s", user_id, e)
            raise
