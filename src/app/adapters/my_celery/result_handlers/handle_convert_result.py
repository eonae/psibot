...
from src.app.core.use_cases import HandleConvertResultUseCase


async def handle_convert_result(ex: Exception | None, _: Any | None, headers: Headers):
    """Обработчик успешного завершения конвертации аудио"""
    job_id = headers.get("job_id")
    assert job_id is not None

    use_case = HandleConvertResultUseCase(
